...
namespace Tokkepedia.Models
{
    ...
    public static class CountryTool
    {
        ...
        public static List<StateModel> GetUnitedStates()
        {
            List<StateModel> states = new List<StateModel>();
            foreach (var map in _stateMappings)
            {
                states.Add(new StateModel() { Id = map.Key, Name = map.Value, Country = "United States", Image = GetStateFlagPNG4x3(map.Key) });
            }

            return states;
        }

        public static List<StateModel> GetCountryStates(string countryId = "us")
        {
            List<StateModel> states = new List<StateModel>();
            if (countryId == "us")
            {
                foreach (var map in _stateMappings)
                {
                    states.Add(new StateModel() { Id = map.Key, Country = "United States", Name = map.Value, Image = GetStateFlagPNG4x3(map.Key) });
                }
            }
...
//public static List<CountryModel> GetCountryStates(string countryId = "us")
//{
//    List<CountryModel> states = new List<CountryModel>();
//    if(countryId == "us")
//    {
//        foreach (var map in _stateMappings)
//        {
//            states.Add(new CountryModel() { Id = map.Value, Name = "United States", State = map.Key });
//        }
//    }
...
//        if (CountryList.FirstOrDefault(x => x.Name == R.EnglishName) == null)
//        {
//            CountryModel c = new CountryModel();
//        c.Id = R.TwoLetterISORegionName.ToLower();
//            c.Name = R.EnglishName;
...
//    {
//        text += "{\"" + country.Id +  "\", \"" + country.Name +  "\"}," + Environment.NewLine;
//    }
